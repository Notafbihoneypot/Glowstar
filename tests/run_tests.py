...
btm=(JAVA/'BluetoothMeshManager.java').read_text(); http=(JAVA/'LocalHttpServer.java').read_text(); proto=(JAVA/'Protocol.java').read_text(); svc=(JAVA/'MeshService.java').read_text(); gradle=(ROOT/'android-helper/app/build.gradle').read_text(); bridge=(JAVA/'AndroidBridge.java').read_text(); qrscan=(JAVA/'QrScanActivity.java').read_text()
...
ok('Native offline QR scanner', all(x in qrscan for x in ['CameraManager','ImageReader','MultiFormatReader','BarcodeFormat.QR_CODE']))
ok('QR camera teardown serialized', all(x in qrscan for x in ['requestCameraClose()', 'closeCameraOnThread()', 'cameraGeneration', 'decodeExecutor', 'duplicate()']) and 'closeCamera();\n        stopCameraThread();' not in qrscan)
ok('QR decoder never uses ImageReader buffers asynchronously', 'byte[] y = copyYPlane(yPlane, width, height);' in qrscan and 'decodeExecutor.execute' in qrscan)
ok('QR camera duplicate-open guard', 'cameraOpening' in qrscan and 'camera != null || cameraOpening' in qrscan)
...
ok('QR center-crop and full-frame decoding', all(x in qrscan for x in ['crop62W', 'crop84W', 'decodeRegion(data, width, height, 0, 0, width, height', 'GlobalHistogramBinarizer']))
