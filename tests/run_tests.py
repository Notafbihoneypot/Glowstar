...
ok('Login persistence follows visible checkbox state', 'function glowstrApplyLoginPersistChoice()' in text and "document.getElementById('login-persist-checkbox')" in text and all(x in text for x in ['async function loginWithKey() {\n  glowstrApplyLoginPersistChoice();','async function generateKeys() {\n  glowstrApplyLoginPersistChoice();','function loginWithAmber() {\n  glowstrApplyLoginPersistChoice();']))
ok('Legacy public-only session probes signer vault', "glowstrRestoreNativeLocalSigner('');" in text and "Migration from 0.3.8/0.3.9 public-only state" in text)
ok('Signer restore tolerates missing legacy method', "function glowstrRestoreNativeLocalSigner(savedMethod='')" in text and "(savedMethod && method !== savedMethod)" in text)
ok('Missing signer exposes reconnect UI', 'id="reconnect-signer-btn"' in text and 'function glowstrReconnectSigner()' in text and "reconnectBtn.onclick = signerReady ? null : glowstrReconnectSigner" in text)
ok('Logout synchronizes persistence UI', "state.persistKey = false;" in text and "glowstrSyncPersistUi();\n  showToast('Disconnected. Identity cleared.')" in text)
...
ok('Reply button uses delegated dynamic action', 'data-glow-action="start-reply"' in text and "case 'start-reply': startReply(el.dataset.eventId, el.dataset.pubkey); break;" in text)
ok('Mobile reply opens mobile composer', "if (mobile && typeof openMobileComposer === 'function')" in text and 'openMobileComposer();' in text[text.index('function startReply'):text.index('function cancelReply')])
ok('Reply context survives mobile composer open', "state.replyTo = {" in text[text.index('function startReply'):text.index('function cancelReply')] and "replyContext.classList.add('visible')" in text[text.index('function startReply'):text.index('function cancelReply')])
ok('Reply validates event and author ids', "Could not open reply composer for this note" in text and "targetPubkey = pubkey || event?.pubkey || ''" in text)
